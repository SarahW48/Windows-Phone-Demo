    ...
    public class GenerateTileInfo
    {
        ...
        static string[,] AppDescription = { // the descriptions of each app
                                              ...
                                              {//Organization
                                                  ...
                                                  "A simple timer and stopwatch app for Windows Phone. It uses the background capabilities of Windows Phone 7.5 Mango and is able to show alerts even if the app is closed. It is also able to show the next alarm on its live tile.",
                                                  "All My Passwords is the most professional trusted passwords app made for Window Phone 7, all your data will be stored with highly encrypted script to ensure privacy. All My Passwords is a very organized and user friendly app; where you can create folders then add passwords in these folders; even every password has a special category with a special icon.",
                                                  "Network Dashboard provides a quick, at a glance view into the network status on your Windows Phone. Tap on a tile and you can quickly change the settings. Tap and hold a tile and you can pin it to the Start menu for quick and easy access!",
                                                  "Craigslist is an extremely fast and accurate Craigslist application on your Windows Phone 7.",
                                                  ...
                                                  "Create a note and pin it to your home screen as a great reminder!",
                                                  ""
                                              },
                                            //  Free Ringtones is a biggest collection of free ringtones available on Window Phone Store. 
                                          };
        ...
        public Dictionary<int, String[]> generate(ScreenTemplate currentTemplate, int templateIndex, string color, double[,] input, string game, string family)
        {
            ...
            bool setfirst = false;
            if ((input[2, 1] != 0) || (input[4, 1] != 0))//if not music or camera or game, then filled with mail
            {
                setfirst = true;
            }
            ...
            {
                if (setfirst)//filled with filled with mail
                {
                    setfirst = false;
                ...
                {
                    randonapp = random.Next(1, 100);//generate an random number
                    thredup = 0;
                    for (int i = 0; i < appNum; i++)
                    {
                        thredup += input[i, 1];
                    ...
                    else if (controller == 1)
                    {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < appNum; i++)
                            {
                                ...
                                thredup += input[i, 1];
                                if (randonapp < thredup)
                                {   // the appIndex[i] in the  AppCategories[i] apps
                                    path = AppCategories[i] + "/" + appIndex[i].ToString() + ".png";
                                    name = AppName[i, appIndex[i]];
                                    tmpsmallpath[smallpathindex] = path + "," + color + "," + pair.Key.x + "," + pair.Key.y + ",1," + name + ",";
                                    Console.WriteLine(tmpsmallpath[smallpathindex]);
